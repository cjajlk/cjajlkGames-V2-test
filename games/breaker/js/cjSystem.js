(function () {
    ...
    function getGameState() {
        // ✅ Breaker: running = true pendant le gameplay, false au menu/fin
        // pause n'existe pas dans Breaker (pas de pause mid-game)
    ...
    function isActiveSession() {
        ...
        return !!(state.running && document.visibilityState === "visible");
    }

    function rewardCJ(amount) {
        // ✅ Sync vers CJajlkAccount (compte global)
        if (window.CJajlkAccount && typeof window.CJajlkAccount.add === "function") {
            window.CJajlkAccount.add("breaker", amount);
            if (isDebugEnabled()) {
                console.log("🎮 CJ sync -> CJajlkAccount.add(breaker,", amount, ")");
            }
        }
        ...
        if (isDebugEnabled()) {
            console.log("✅ Breaker earned", amount, "CJ");
        }
    }
